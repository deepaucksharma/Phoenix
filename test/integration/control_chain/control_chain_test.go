...
func TestControlChainPatchFlow(t *testing.T) {
	t.Skip("TODO: implement integration once components support full patch flow")
	...
	patch := interfaces.ConfigPatch{
		PatchID:             "patch1",
		TargetProcessorName: topkSettings.ID,
	...
	// Process metrics: combine patch metric with regular metrics
	metrics := testutils.GenerateMetrics()
	metrics.ResourceMetrics().MoveAndAppendTo(patchMetrics.ResourceMetrics())

	err = topkProc.ConsumeMetrics(ctx, patchMetrics)
	require.NoError(t, err)

	// Check that priority_tagger added priority attributes
	require.Greater(t, sink.DataPointCount(), 0)

	// Verify k_value was updated via patch
