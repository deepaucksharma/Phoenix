...
init_environment() {
    ...
    if [ ! -f config/defaults/control/optimization_mode_template.yaml ]; then
        cat > config/defaults/control/optimization_mode_template.yaml << 'EOF'
...
update_configs() {
    ...
    if [ -f monitoring/prometheus/rules/phoenix_core_rules.yml ]; then
        # Ensure Prometheus loads the new rules
        if ! grep -q "phoenix_core_rules.yml" monitoring/prometheus/prometheus.yaml; then
            echo "    - '/etc/prometheus/rules/phoenix_core_rules.yml'" >> monitoring/prometheus/prometheus.yaml
